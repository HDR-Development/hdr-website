# ![](../../images/Stock_IceClimbers.png) Ice Climbers

<link rel="stylesheet" type="text/css" rel="noopener" target="_blank" href="../../style.css">

![](../../images/SmashBall.png) <b>Attributes</b>
  - Popo
    - Initial Dash Speed: 1.68 🠚 1.45
    - Run Speed: 1.53 🠚 1.45
    - Run Acceleration (base/additional): 0.044/0.08096 🠚 0.02/0.05
    - Ground Friction: 0.077 🠚 0.035
    - Jumpsquat: 3F 🠚 4F
    - Horizontal Jump Speed Multiplier: 0.57 🠚 0.6
    - Short Hop Height: 16.75 🠚 14.75
    - Air Speed: 0.83 🠚 0.875
    - Air Acceleration (base/additional): 0.01/0.08 🠚 0.01/0.06
    - Air Friction: 0.015 🠚 0.02
    - Gravity: 0.082 🠚 0.1
    - Fall Speed: 1.3 🠚 1.61
    - Fast Fall Speed: 2.08 🠚 2.167
    - Combo Gravity: 0.072912 🠚 0.1
    - Combo Fall Speed: 1.8 🠚 1.61
    - Heavy Landing Lag: 7F 🠚 4F
  - Nana
    - Walk Speed: 1.008 🠚 1.108
    - Initial Dash Speed: 1.68 🠚 1.45
    - Run Speed: 1.683 🠚 1.545
    - Run Acceleration (base/additional): 0.0484/0.089056 🠚 0.02/0.05
    - Ground Friction: 0.0847 🠚 0.0385
    - Jumpsquat: 3F 🠚 4F
    - Max Horizontal Jump Speed: 0.57 🠚 0.6
    - Short Hop Height: 16.75 🠚 14.75
    - Air Speed: 0.8715 🠚 0.91875
    - Air Acceleration (base/additional): 0.0105/0.084 🠚 0.0105/0.0.6355
    - Air Friction: 0.015 🠚 0.02
    - Gravity: 0.082 🠚 0.1
    - Fall Speed: 1.3 🠚 1.61
    - Fast Fall Speed: 2.08 🠚 2.167
    - Combo Gravity: 0.072912 🠚 0.1
    - Combo Fall Speed: 1.8 🠚 1.61
<br><br>

![](../../images/SmashBall.png) <b>Moveset</b>
  - Nana:
    - <property>Popo’s buffer is now inactive during spotdodges and rolls, allowing for Nana to buffer an action during them to desync the Ice Climbers</property>
    - <buff>Nana is now actionable during all throws</buff>
    - Damage/Knockback Multiplier: <buff>1.05x 🠚 1.0x</buff>
    - Internal CPU Level Modifier: <buff>76 🠚 100</buff>
<br><br>
  - Jab:
    - Jab 1
      - Popo
        - Damage: <buff>2.0% 🠚 2.5%</buff>
        - BKB: <buff>20 🠚 25</buff>
        - Hitbox Size (base/mid/tip): <buff>2.0/2.2/2.4u 🠚 2.5/3.0/3.5u</buff>
      - Nana
        - BKB: <buff>20 🠚 25</buff>
        - Hitbox Size (base/mid/tip): <buff>1.8/2.0/2.3u 🠚 2.5/3.0/3.5u</buff>
      - FAF: <buff>28 🠚 18</buff>
    - Jab 2
      - FAF: <buff>30 🠚 21</buff>
<br><br>
  - Forward Tilt:
    - Popo
      - Damage: <nerf>9.0% 🠚 8.0%</nerf>
      - KBG: <buff>100 🠚 110</buff>
    - Nana
      - Damage: <buff>6.75% 🠚 8.0%</buff>
      - KBG: <buff>100 🠚 110</buff>
    - FAF: <nerf>30 🠚 33</nerf>
<br><br>
  - Up Tilt:
    - <buff>Added scoop hitbox</buff>
    - Scoop
      - Hitbox Duration: F6-11
      - Damage: 1.0%
      - Angle: 125
      - FKB: 75
      - KBG: 100
      - Hitbox Size: 3.5u
    - Multihits
      - Popo
        - Damage: <nerf>0.8% 🠚 0.2%</nerf>
      - Nana
        - Damage: <nerf>0.6% 🠚 0.2%</nerf>
      - Hitlag Multiplier: <adjust>0.5x 🠚 0.75x</adjust>
    - Launcher
      - Popo
        - BKB: <adjust>40 🠚 47</adjust>
        - KBG: <adjust>100 🠚 95</adjust>
        - Hitlag Multiplier: <adjust>2.0x 🠚 1.5x</adjust>
      - Nana
        - Damage: <buff>3.0% 🠚 4.0%</buff>
        - BKB: <adjust>38 🠚 47</adjust>
        - KBG: <adjust>118 🠚 95</adjust>
        - Hitlag Multiplier: <adjust>2.8x 🠚 1.5x</adjust>
<br><br>
  - Down Tilt:
    - <buff>Added shoulder hitbox</buff>
    - Hitbox Duration: <buff>F8-11 🠚 F6-9</buff>
    - Popo
      - Angle: <rework>20 🠚 75</rework>
      - KBG: <buff>100 🠚 95</buff>
    - Nana
      - Damage: <buff>4.5% 🠚 6.0%</buff>
      - Angle: <rework>20 🠚 75</rework>
      - KBG: <buff>100 🠚 95</buff>
    - Hitbox Size (shoulder/base/mid/tip): <buff>*/3.0/3.0/4.0u 🠚 4.0/4.0/4.0/5.0u</buff>
    - FAF: <buff>31 🠚 29</buff>
<br><br>
  - Dash Attack:
    - BKB: <buff>90 🠚 80</buff>
    - Hitlag Multiplier (Popo): <adjust>1.2x 🠚 1.0x</adjust>
    - FAF: <nerf>40 🠚 42</nerf>
<br><br>
  - Forward Smash:
    - <buff>Added hitbox to base of hammer</buff>
    - Hitbox Duration: <buff>F11-13 🠚 F10-14</buff>
    - Popo
      - Damage: <buff>12.0% 🠚 12.5%</buff>
      - KBG: <buff>105 🠚 110</buff>
    - Nana
      - Damage: <buff>9.0% 🠚 12.5%</buff>
      - KBG: <nerf>126 🠚 110</nerf>
    - Hitbox Size (base/mid/tip): <buff>*/4.0/5.5u 🠚 3.0/4.0/6.0u</buff>
    - FAF: <nerf>47 🠚 51</nerf>
<br><br>
  - Up Smash:
    - Hitbox Duration: <buff>F12-17 🠚 F9-15</buff>
    - Popo
      - KBG: <buff>105 🠚 110</buff>
    - Nana
      - Damage (hammer/body): <buff>8.25/6.75% 🠚 11.0/9.0%</buff>
      - KBG: <buff>126 🠚 110</nerf>
    - Hitbox Size (tip): <buff>4.0u 🠚 5.0u</buff>
    - FAF: <nerf>50 🠚 52</nerf>
<br><br>
  - Down Smash:
    - <buff>Animation adjusted so that Popo and Nana hit both forwards and backwards</buff>
    - Hitbox Duration: <buff>F9-11 🠚 F8-11</buff>
    - Popo
      - Damage: <nerf>13.0% 🠚 12.0%</nerf>
      - Angle: <rework>60 🠚 40</rework>
      - KBG: <buff>100 🠚 110</buff>
    - Nana
      - Damage: <buff>9.75% 🠚 12.0%</buff>
      - Angle: <rework>60 🠚 40</rework>
      - KBG: <nerf>120 🠚 110</nerf>
    - FAF: <nerf>41 🠚 45</nerf>
<br><br>
  - Neutral Air:
    - <buff>Added shoulder hitbox</buff>
    - Nana
      - Damage: <buff>5.25% 🠚 7.0%</buff>
      - KBG: <nerf>120 🠚 100</nerf>
    - Hitbox Size (tip): <buff>4.0u 🠚 4.5u</buff>
    - Landing Lag: <nerf>7F 🠚 8F</nerf>
<br><br>
  - Forward Air:
    - Hitbox Duration: <buff>F19-20 🠚 F17-21</buff>
    - Popo
      - <buff>Added spike on frame 18</buff>
      - Sweetspot
        - Damage: 12.0%
        - Angle: 270
        - BKB: 20
        - KBG (ground/air): 100/66
        - Hitbox Size: 4.5u
      - Sourspot
        - KBG: <buff>95 🠚 103</buff>
    - Nana
      - Sweetspot
        - KBG (ground/air): <nerf>100 🠚 100/66</nerf>
      - Sourspot
        - Damage: <buff>9.0% 🠚 12.0%</buff>
        - KBG: <nerf>114 🠚 103</nerf>
    - Autocancel: <buff>F1-2/F49 🠚 F1-3/F49</buff>
<br><br>
  - Back Air:
    - <buff>Added shoulder hitbox</buff>
    - Popo
      - Damage: <nerf>10.0% 🠚 9.0%</nerf>
      - KBG: <buff>115 🠚 134</buff>
    - Nana
      - Damage: <buff>7.5% 🠚 9.0%</buff>
      - KBG: <nerf>138 🠚 134</nerf>
    - Hitbox Size (tip): <buff>4.0u 🠚 4.5u</buff>
    - FAF: <buff>36 🠚 32</buff>
    - Landing Lag: <nerf>7F 🠚 9F</nerf>
<br><br>
  - Up Air:
    - <buff>Added shoulder hitbox</buff>
    - Hitbox Duration: <buff>F7-11 🠚 F7-12</buff>
    - Popo
      - Damage: <nerf>9.0% 🠚 8.0%</nerf>
      - BKB: <adjust>20 🠚 25</adjust>
      - KBG: <adjust>129 🠚 141</adjust>
    - Nana
      - Damage: <buff>6.75% 🠚 8.0%</buff>
      - BKB: <adjust>20 🠚 25</adjust>
      - KBG: <adjust>154.8 🠚 141</adjust>
    - Hitbox Size (shoulder/base/tip): <buff>*/3.2/4.6u 🠚 3.5/3.5/5.0u</buff>
    - Autocancel: <nerf>F1-6/F27 🠚 F1-6/F28</nerf>
    - Landing Lag: <buff>14F 🠚 12F</buff>
<br><br>
  - Down Air:
    - <rework>Removed stall and fall</rework>
    - Hitbox Duration: <buff>F12-51 🠚 F4-45</buff>
    - Popo
      - Damage: <buff>8.0% 🠚 9.0%</buff>
      - Angle: <rework>70 🠚 361</rework>
      - KBG: <buff>90 🠚 95</buff>
      - Hitbox Size: <nerf>6.5u 🠚 6.0u</nerf>
    - Nana
      - Damage: <buff>6.0% 🠚 7.0%</buff>
    - FAF: <buff>64 🠚 56</buff>
    - Autocancel: <buff>F54 🠚 F1/F46</buff>
    - Landing Lag: <buff>20F 🠚 13F</buff>
<br><br>
  - Ice Shot (Neutral Special):
    - Startup: <buff>F18 🠚 F11</buff>
    - FAF: <buff>56 🠚 48</buff>
<br><br>
  - Squall Hammer (Side Special):
    - Single Climber
      - Initial Height Gain: <buff>0.6 🠚 1.3</buff>
      - Gravity: <buff>0.1 🠚 0.08</buff>
    - Double Climber
      - Initial Height Gain: <buff>1.2 🠚 1.35</buff>
      - Gravity: <buff>0.1 🠚 0.087</buff>
<br><br>
  - Belay (Up Special):
    - Distance for Nana to warp to Popo: <nerf>60 🠚 50</nerf>

<script src="../../arrow.js">
</script>